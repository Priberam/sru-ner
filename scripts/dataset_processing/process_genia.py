"""
Adapted from
https://github.com/yhcc/CNN_Nested_NER/blob/master/preprocess/process_genia.py
"""
...
class Sentence(object):
    def __init__(self, sentence_xml):
        ...
        self.span_annotations, self.token_annotations = self.get_annotations(
            sentence_xml
        )
    ...
    def get_tokens(sentence_xml):
        ...
        if text != _text:
            ...
            tokens.append(
                Token(
                    left_text, left_text, idx, idx + len(left_text), "", " ", "*", "*"
                )
            )
        return tokens
    ...
    def process_annotation(
        parent_annotation, text, span_annotations, idx, allow_nest=False
    ):
        ...
        for child in parent_annotation.childGenerator():
            ...
            tokens += _tokens
        for _idx, annotation in enumerate(
            parent_annotation.find_all("cons", recursive=False)
        ):
            ...
            if found_match is None:
                raise (
                    "1:Cannot find {} in {} at {} ({})".format(
                        annotation.get_text(), text[idx:], idx, text
                    )
                )
            else:
            ...
            Sentence.process_annotation(
                annotation, text, span_annotations, ann_idx, allow_nest=True
            )
            try:
                ann_lex = annotation["lex"]
            except:
                ann_lex = r" ".join(
                    [tok.text for tok in Sentence.get_tokens(annotation)]
                )
            ...
            if len(sub_cons) > 1 and (
                ...
                for sub_con in sub_cons:
                    sub_con_txt = r" ".join(
                        [tok.text for tok in Sentence.get_tokens(sub_con)]
                    )
                    ...
                    if sub_con_idx == -1:
                        ...
                        print(sub_cons_ann)
                        raise Exception(
                            "2:Cannot find {} in {} at {} ({})".format(
                                sub_con_txt, text[idx:], idx, text
                            )
                        )
                    sub_cons_ann.append(
                        (
                            Sentence.normalize_lex(sub_con["lex"]),
                            Span(sub_con_idx, sub_con_idx + len(sub_con_txt)),
                        )
                    )
                ...
                for sub_lex, sub_sem in sub_anns:
                    ...
                    span_annotations.append(
                        Annotation(spans, sub_sem, text, combined_ann, dis=True)
                    )
            idx = ann_idx + len(ann_txt)
    ...
    def split_lex(lex, idx=None):
        ...
        if idx[0] == len(lex):
            return result
        if lex[idx[0]] == "(" and re.match(
            "^\\((AND|OR|BUT_NOT|AS_WELL_AS|VERSUS|TO|NOT_ONLY_BUT_ALSO|NEITHER_NOR|THAN) .+",
            lex,
        ):
    ...
    def find_spans(lex, text, cons):
        ...
        for con_lex, con_span in cons:
            ...
            if con_lex_idx == -1:
                ...
                if (
                    con_lex_idx != -1
                    and con_lex_idx < lex_idx
                    and len(con_lex) > spans[-1].end - spans[-1].start
                ):
                    print(
                        "Found {} from backing off from {} for {}, please check ({}) {}".format(
                            con_lex, spans[-1].get_text(text), lex, text, cons
                        )
                    )
        ...
        if diff >= 1:
            ...
            if diff == 1:
                print(
                    'WARNING: differ by one: "{}", found: "{}"'.format(
                        lex, lex[:lex_idx]
                    )
                )
            else:
...
def split_train_dev_test(sentences, train_pct=0.8, dev_pct=0.1, test_pct=0.1):
    ...
    return (
        sentences[:train_count],
        sentences[train_count:start_test_idx],
        sentences[start_test_idx:],
    )
        ...
        if remove_over or (remove_disc and ann.parent is not None):
            ...
            for idx in reversed(range(len(result))):
                ...
                if (
                    not remove_over
                    and remove_disc
                    and ann.parent is not None
                    and ann.parent != ann2.parent
                ):
                    continue
                if ann2.overlaps(ann):
                    if ann2.contains(ann):
                        ...
                        # Neither is contained within the other, not nested!
                        # Remove one arbitrarily, easier to remove the latter
                        need_to_be_removed = True
...
def remove_spaces_inside_tokens(data: dict):

    new_data = []

    for sent in data:
        if any(" " in t for t in sent["tokens"]):

            """# Option 1: split into new tokens
            n_tok = []
            tok_map = {}
            n_id = 0
            for o_id, tok in enumerate(sent["tokens"]):
                if " " in tok:
                    sep_tok = tok.split(" ")
                    n_tok.extend(sep_tok)
                    tok_map[o_id] = [n_id + x for x in range(len(sep_tok))]
                    n_id += len(sep_tok)
                else:
                    n_tok.append(tok)
                    tok_map[o_id] = n_id
            ...
            # Option 2: compress spaces
            n_tok = [t.replace(" ", "") for t in sent["tokens"]]
            n_ents = []
            for ent in sent["entity_mentions"]:
                n_ent = {
                    "start": ent["start"],
                    "end": ent["end"],
                    "entity_type": ent["entity_type"],
                    "text": " ".join(n_tok[ent["start"] : ent["end"]]),
                }
                n_ents.append(n_ent)
            ...
            n_sent = {
                ...
                "entity_mentions": n_ents,
            }
            new_data.append(n_sent)
...
def save_simple_format(data: dict, output_path: str):
    ...
    for sent in data:
        this_doc = sent["doc_id"]
        if prev_doc is None or prev_doc != this_doc:
            lines.append(f"-- doc-sep -- ({this_doc})\n")
            prev_doc = this_doc

        # In their original files
        tokens = [t.replace(" ", "") for t in sent["tokens"]]
        mentions = []
        for ment in sent["entity_mentions"]:
            mentions.append(
                LabeledSubstring(
                    start=ment["start"],
                    end=ment["end"],
                    tag=ment["entity_type"],
                    text=ment["text"],
                )
            )

        label_stats += Counter([m.tag for m in mentions])

        simple_ac_seq_aux = list_entities_to_actions_encoding_with_OUT(
            words=tokens, labels=mentions
        )

        text_seq = " ".join(tokens)
        simple_ac_seq = " ".join(simple_ac_seq_aux)
        line = text_seq + " ||| " + simple_ac_seq + "\n"
        lines.append(line)

        # Sanity check
        check = check_and_extract_action_sequence_from_file(
            simple_ac_seq_aux, tokens, "simple"
        )
        if isinstance(check, bool):
            raise ValueError()

    print(f"\nSaving {output_path} with:")
    print("# Sents: ", len(data))
    print("# labels: ", label_stats)

    with open(output_path, "w") as f:
        f.writelines(lines)


def main(xml_path, output_dir, genia_split_path):
    ...
    for split in result_data:
        clean_data = remove_spaces_inside_tokens(result_data[split])

        save_simple_format(
            data=clean_data,
            output_path=os.path.join(output_dir, f"{split}.parser"),
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert GENIA to simple format.")
    parser.add_argument(
        "--xml_path",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path of the folder containing the splits.",
    )
    ...
    main(
        ...
        genia_split_path=args.genia_split_folder,
    )
